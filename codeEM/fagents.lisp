...
(defun agent-cheap-cheaper-in-comps (designs)
...
(defun agent-expensive-cheaper-in-comps (designs)
...
(defun agent-light-lighter-in-comps (designs)
...
(defun agent-heavy-lighter-in-comps (designs)
...
(defun agent-cheap-doubles-in-comps (designs)
...
(defun agent-expensive-doubles-in-comps (designs)
  (let ((design (max-or-min-of-designs 
		 0 (remove-if 
		    #'(lambda (x) (equal 
				   (sc-components x) 
...
(defun agent-light-doubles-in-comps (designs)
  (let ((design (max-or-min-of-designs 
		 1 (remove-if 
		    #'(lambda (x) (equal 
				   (sc-components x) 
...
(defun agent-heavy-doubles-in-comps (designs)
  (let ((design (max-or-min-of-designs 
		 1 (remove-if 
		    #'(lambda (x) (equal 
				   (sc-components x) 
...
(defun agent-cheap-dangles-in-graph (designs)
  (let* ((design 
...
(defun agent-expensive-dangles-in-graph (designs)	 
  (let* ((design (max-or-min-of-designs 
		 0 (remove-if-not
		    #'(lambda (x) (member-if #'(lambda (xx) 
						 (= (length (fp-index xx)) 1))
...
(defun agent-light-dangles-in-graph (designs)
  (let* ((design (max-or-min-of-designs 
		 1 (remove-if-not
		    #'(lambda (x) (member-if #'(lambda (xx) 
						 (= (length (fp-index xx)) 1))
...
(defun agent-heavy-dangles-in-graph (designs)
  (let* ((design (max-or-min-of-designs 
		 1 (remove-if-not
		    #'(lambda (x) (member-if #'(lambda (xx) 
						 (= (length (fp-index xx)) 1))
...
(defun agent-cheap-cheaper-in-graph (designs)
  (let* ((design (max-or-min-of-designs 0 designs :operator 'min))
	 (i (position-of-max-or-min-comp 0 (sc-components design) 
					 :operator 'max)))
...
(defun agent-expensive-cheaper-in-graph (designs)
  (let* ((design (max-or-min-of-designs 0 designs :operator 'max))
	 (i (position-of-max-or-min-comp 0 (sc-components design) 
					 :operator 'max)))
    (cond (i (list (fragment-graph (list i) design) design)))))
...
(defun agent-light-lighter-in-graph (designs)
  (let* ((design (max-or-min-of-designs 1 designs :operator 'min))
	 (i (position-of-max-or-min-comp 1 (sc-components design) 
					 :operator 'max)))
    (cond (i (list (fragment-graph (list i) design) design)))))
...
(defun agent-heavy-lighter-in-graph (designs)
  (let* ((design (max-or-min-of-designs 1 designs :operator 'max))
	 (i (position-of-max-or-min-comp 1 (sc-components design) 
					 :operator 'max)))
    (cond (i (list (fragment-graph (list i) design) design)))))
...
(defun agent-cheap-doubles-in-graph (designs)
  (let ((design (max-or-min-of-designs 
		 0 (remove-if 
...
(defun agent-expensive-doubles-in-graph (designs)
  (let ((design (max-or-min-of-designs 
		 0 (remove-if 
		    #'(lambda (x) (= (length (sc-components x))
				     (length (remove-duplicates (sc-components x)))))
		    designs) :operator 'max)))
    (cond 
     (design
      (do ((double-comp
...
(defun agent-light-doubles-in-graph (designs)
  (let ((design (max-or-min-of-designs 
		 1 (remove-if 
		    #'(lambda (x) (= (length (sc-components x))
				     (length (remove-duplicates (sc-components x)))))
...
(defun agent-heavy-doubles-in-graph (designs)
  (let ((design (max-or-min-of-designs 
		 1 (remove-if 
		    #'(lambda (x) (= (length (sc-components x))
				     (length (remove-duplicates (sc-components x)))))

          ...
          (t
           (let ((index (nth (random (length (cdr best))) (cdr best))))
             (update-config 
              ...
              (mod-list (mod-list (make-list (1+ (nth (fourth index) port-maxes)))
				  (third index) 
...
(defun search-for-best-U (fps ebs set-fps eb-lib connect-fps prefs domain dir
			      sp connect todo taboo c-fp-max
...
(defun calc-c-agent-utility (fps ebs set-fp eb c-fp set-port-i connect-port-i
                                 prefs domain dir sp connect todo taboo)
	...
	(t 
	 (+ 
	  (* (first prefs)  (U-of-set-fp set-fp dir sp domain))
	  (* (second prefs) 
	      (U-of-eb-port set-fp (eval eb) set-port-i domain))
	  (* (third prefs)
	  ...
	  (* (sixth prefs)
	       (print (U-of-goal-completeness ebs fps set-fp eb c-fp set-port-i connect-port-i)))))))


(defun U-of-set-fp (fp dir sp domain)
  ;; + 1 for containing a goal state variable (oper is non-nil)
  ;; NOT ANY MORE! we will do this in our new function, U-of-goal-completeness
  ;; + 1 for being to the proper domain
  ;; + 1 for being of the proper direction
  ;; + 1 for being proper serial or parallel(defun create-configs (c-agents new-num &optional todo taboo)
  (cond 
   ((<= new-num 0) 
    (format t "~%"))
   (t
    (let* ((config (print (create-from (mapcar 'copy-fp *io-fps*) 
				c-agents todo taboo)))
	   (equations 
	     (cond 
	     ((first config) 
 ...
 ;      (t (format t "-") (create-configs c-agents new-num todo taboo)))))))
    ;   (t  (create-configs c-agents new-num todo taboo))
       )))))

  (/ (+ 
      (cond ((equal domain (fp-domain fp)) 1) (t 0))
...
(defun U-of-eb-port (set-fp eb set-port-num domain)
  ;; + 1 for being to the proper domain
...
(defun U-of-goal-completeness (ebs fps set-fp eb c-fp set-port-i connect-port-i)
  (let* ((new-possible-design  (update-config 
				eb
				(mod-list 
				 (mod-list 
				  (make-list 
				   (length (eb-domain (eval eb))))
				  set-port-i set-fp)
				 connect-port-i c-fp)
				ebs
				fps))
         (new-goals-num (/ (goal-count (car new-possible-design))
                           (length (car new-possible-design))))
         (old-goals-num (/ (goal-count fps) (length fps))))
    (cond ((> old-goals-num new-goals-num)
           (/ (- old-goals-num new-goals-num) old-goals-num))
    (t 0)))
...
(defun goal-count (fps)
  (cond ((endp fps) 0)
        (t
         (+ 
          (r-count 'goal (fp-through (car fps)))
          (r-count 'goal (fp-across (car fps)))
          (goal-count (cdr fps))))))
...
;;;     (list fps (backcons eb ebs))
     ;;;   nil
;;;     (print best) (print fps) (print ebs)
...
(defun calc-c-agent-utility (fps ebs set-fp eb c-fp set-port-i connect-port-i
	...
	(t 
	 (+ 
	  ...
	  (* (sixth prefs)
      (U-of-goal-completeness ebs fps set-fp eb c-fp set-port-i connect-port-i))
   ))))
...
(defun U-of-eb-port (set-fp eb set-port-num domain)
  ...
  ;; + 1 for being of the proper state-var operator goal fulfillment
    ;; NOT ANY MORE! we will do this in our new function, U-of-goal-completeness
;  (/ (+ 
      (cond ((member domain (eb-domain eb) :test 'equal) 1)
     	      (t 0)))
;	(cond ((and oper
;		    (= (mod oper 4)

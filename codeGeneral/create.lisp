...
(defun create-and-modify-designs (new-num mod-set c-agents i-agents f-agents
  ...
  (do* ((agent nil (M-find-next-agent-M f-agents))  ; todo taboo)
       ...
       ((null mod-set)  (print new-num) (print mod-set) 
   ...
   (t
	...
	(t 
	 ...
	 (reconstruct-fragments 
	  c-agents fragments todo taboo 
	  (cycle1+ attempt *attempts-to-reconstruct*))))))))
...
(defun create-from (fps c-agents todo taboo &optional ebs c-team)
  (cond ((design-is-complete fps) (list fps ebs c-team))
	((>= (length ebs) *max-num-ebs*) (list nil nil nil))
	(t
	 (let* ((agent (M-find-next-agent-M c-agents c-team todo taboo))
		(d-state  (c-agent-strategy fps ebs :prefs (second agent) 
					    :domain (third agent)
					    :dir (fourth agent) :sp (fifth agent) 
					    :connect (sixth agent) :todo todo 
					    :taboo taboo)))
	   (print d-state)
	   (cond (d-state (create-from  (car d-state) c-agents todo taboo
							  (cadr d-state) (backcons agent c-team)))
		 (t (format t "there") (list nil nil nil)))))))

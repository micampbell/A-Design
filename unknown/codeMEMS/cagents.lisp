...
(defun c-agent-strategy (fps ebs &key prefs domain dir sp connect todo taboo)
  (let* ((set-fps (remove-if #'(lambda (x) 
				 (or (member 'ground (fp-index x) :test 'equal)
				     (null (fp-inter x)))) fps))
	 ;;(eb-lib *eb-library*)
	 ;; The following is specific hack for MEMS in order to avoid joints
	 (eb-lib (butlast *eb-library*))
	 ...
	 (port-maxes (mapcar #'(lambda (x) (1- (length (eb-const-param
							(eval x))))) 
			     eb-lib))
	  ...
	  (t
	   (let ((index (nth (random (length (cdr best))) (cdr best))))
	     (update-config 
	      ...
	      (append (make-list (second index))
		      (cons (nth (first index) c-fps)
			    (make-list (- (nth (fourth index) port-maxes)
					  (second index)))))
	      ebs
	      fps))))))
...
(defun calc-c-agent-utility (fps ebs set-fp eb c-fp set-port-i connect-port-i
  ...
  (cond ((or (= set-port-i connect-port-i) 
	     (equal set-fp c-fp)
	     (not (match-cp-fp (nth set-port-i (eb-const-param (eval eb))) 
			       set-fp))
	     (and c-fp (not (match-cp-fp (nth connect-port-i 
	...
	;;				set-port-i c-fp connect-port-i taboo))
	  (* (fifth prefs) 0)))))
	  ;;   (member-of-todo-taboo-list fps (backcons eb ebs) set-fp
		;;			set-port-i c-fp connect-port-i
			;;		todo))))))
...
(defun U-of-eb-port (set-fp eb set-port-num oper domain)
  ...
  (/ (+ (cond ((member domain (mapcar #'cp-domain (eb-const-param eb))
		       :test 'equal) 
	       1)
	      (t 0))
	(cond ((and oper
...
(defun U-of-connect-fp (connect-fp set-fp set-port-num connect-port-num eb 
				   sp connect oper)
  ;; + 1 for proper connection type (ground, other, nil)
   ...
   (+ 
    ...
    (cond ((null oper) 0)
	  ...
	  ((null (nth (truncate oper 4)
		      (cp-oper (nth set-port-num (eb-const-param eb)))))
...
;(defun member-of-todo-taboo-list (fps ebs set-fp set-eb set-port-num c-fp list)
;  (let ((candidate (construct
;		    (cond ((or-list (mapcar #'(lambda (y) (or (and-list (member(subgraphp x y)) taboo))
;						))))))))))
...
;;; MATCH-CP-FP
;;; Predicate which judges if a port or cp connects to an fp
(defun match-cp-fp (c f)
  (and c f
       (or (not (cp-class c))
	   (not (fp-class f))
	   (equal (cp-class c) (fp-class f)))
       (or (not (cp-domain c)) 
	   (not (fp-domain f))
	   (equal (cp-domain c) (fp-domain f)))
       (match-interface (cp-inter c) (fp-inter f))))


(defun match-interface (inter1 inter2)
  (and inter1 inter2
       (or (equal inter1 '(west north east south up down))
	   (equal inter2 '(west north east south up down))
       (let ((inter1 (set-difference inter1 inter2))
	     (inter2 (set-difference inter2 inter1)))
	 (or (and (member 'east inter1) (member 'west inter2))
	     (and (member 'east inter2) (member 'west inter1))
	     (and (member 'north inter1) (member 'south inter2))
	     (and (member 'north inter2) (member 'south inter1))
	     (and (member 'up inter1) (member 'down inter2))
	     (and (member 'up inter2) (member 'down inter1)))))))
